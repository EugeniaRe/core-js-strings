 ...
 */
function concatenateStrings(concvalue1, concvalue2) {
  const concat = `${concvalue1}${concvalue2}`;
  return concat;
}
...
function isPalindrome(str) {
  const str1 = str.replace(/[^a-zа-яё]/gi, '').toLowerCase();
...
function reverseWords(str) {
  ...
  strArray.forEach((element) => {
    resArray.push(element.split('').reverse().join(''));
  });
...
function encodeToRot13(str) {
  return str
    .split('')
    .map((element) => {
      const elementCode = element.charCodeAt(element);
      if (
        elementCode < 65 ||
        (elementCode > 90 && elementCode < 97) ||
        elementCode > 122
      ) {
        return String.fromCharCode(elementCode);
      }
      if (elementCode < 78 || (elementCode > 96 && elementCode < 110)) {
        return String.fromCharCode(elementCode + 13);
      }
      return String.fromCharCode(elementCode - 13);
    })
    .join('');
